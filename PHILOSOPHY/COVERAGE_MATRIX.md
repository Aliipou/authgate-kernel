...
| 8 | **Consent Logic** | `valid_consent(H,A) :- informed, voluntary, specific, revocable, competent, not coerced, not deceived` | [`kernel/consent.py`](../src/authgate/kernel/consent.py), [`kernel/consent_registry.py`](../src/authgate/kernel/consent_registry.py), [`extensions/consent_semantics.py`](../src/authgate/extensions/consent_semantics.py) | **Partial** — *specific, revocable, expiring, human-grantor* enforced in `kernel/`. *informed / voluntary / competent / not-deceived* require semantics, which the TCB correctly refuses to fake — an optional, untrusted, veto-only, fail-closed judge composed outside the gate (`semantic_consent_veto`) now covers them when a caller opts in. Still **Partial** on purpose: a judge's permit is not a mechanical guarantee and the status word says so. See [`../docs/CONSENT_SEMANTICS.md`](../docs/CONSENT_SEMANTICS.md). |
...
| 20 | **God -> Human (ontological root)** | `Person(h) -> OwnedByGod(h)` | — | **Documented gap, permanently** — the human is the authority root; the divine tier is not modeled in the TCB, and could not be by any formal system without a category error. Why this is the correct place to stop, not an oversight to fix, is argued in [`GOD_HUMAN_BOUNDARY.md`](GOD_HUMAN_BOUNDARY.md). |
